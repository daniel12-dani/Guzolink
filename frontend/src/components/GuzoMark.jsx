/**
 * GuzoMark — the footprint brand icon, as a scalable inline SVG.
 *
 * Two variants:
 *  - <GuzoMark />              → footprint in a filled circle (navbar / avatar use)
 *  - <GuzoMark plain />        → just the footprint, no circle (for backgrounds/watermarks)
 *
 * Colors are passed as Tailwind classes so it always matches the current
 * theme instead of shipping a fixed color baked into a PNG.
 */
function GuzoMark({
  size = 32,
  plain = false,
  ...
  return (
    <svg
      ...
      aria-hidden="true"
    >
      <circle cx="256" cy="256" r="256" className={circleClassName} />
      <g transform="translate(70,70) scale(0.72)">{foot}</g>
    </svg>
  );
}

export default GuzoMark;