...
export default function HomePageProductCard({ product }) {
  ...
  // Since we fetch "createdBy" via GraphQL, check if it matches the current user
  const creatorId = product.createdBy?.id || product.createdBy?._id;
  ...
  return (
    <article
      className={`group flex flex-col overflow-hidden rounded-xl border p-2 transition hover:-translate-y-0.5 
        ${
          isMyProduct
    ...
    >
      <Link to={`/products/${product.id}`} className="relative block">
        ...
        <div className="absolute right-1 top-1 flex flex-col gap-1">
          {isMyProduct && (
            <span className="rounded bg-amber-500/90 px-1.5 py-0.5 text-[10px] font-bold text-slate-900 shadow">
              Your Product
            </span>
          )}
          {!inStock && (
      ...
      <Link
        ...
        className="mt-2 line-clamp-2 min-h-10 text-sm text-slate-200 hover:text-white"
      >
        {product.name}
      </Link>
      ...
      <div className="mt-1.5 flex items-center justify-between">
        ...
        {inStock &&
          (isMyProduct ? (
            // Disabled state style for owners to prevent self-purchases
            <div
              title="You cannot add your own products to your cart"
              className="flex h-7 px-2 items-center justify-center rounded-full bg-slate-800 text-slate-400 text-xs font-medium cursor-not-allowed border border-slate-700"
            >
              Owner
            </div>
          ) : (
